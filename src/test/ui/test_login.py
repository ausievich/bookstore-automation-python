...
class TestLogin:
    def setup_method(self) -> None:
        allure_metadata(layer=AllureLayer.UI, suite="User Login")

    @allure.title(f"{tms_link('C1001')} valid login shows catalog dashboard")
    ...
    @allure.title(f"{tms_link('C1002')} invalid password shows error")
    ...
    @allure.title(f"{tms_link('C1003')} empty fields show validation messages")
